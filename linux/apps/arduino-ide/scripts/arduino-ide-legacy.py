...
def main():
    ...
    base_url = "https://downloads.arduino.cc/"
    search = "Linux 64 bits"
    symlink_name = "arduino-legacy-latest.tar.xz"
    max_age = 86400 
    
    timestamp_not_exceeded = check_latest_timestamp(args.path, symlink_name, max_age)

    if timestamp_not_exceeded:
        remote_url, file_name = get_remote_info(url, base_url, search)
        is_latest_file = check_file_version(args.path, file_name)
    else:
        print("Latest file for %s is too new, skipping." % symlink_name)
        sys.exit(0)

    if not is_latest_file:
        retrieve_file(remote_url, os.path.join(args.path, file_name))
        create_symlink(file_name, os.path.join(args.path, symlink_name))
